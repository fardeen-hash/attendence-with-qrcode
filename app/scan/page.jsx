...
function Scan() {
  ...
  const clear = () => {
    ...
    const addattendence = async (e) => {
      console.log(data);
      const data2 = data.replaceAll('"', '');
      setOpen(false);
      try {
        const response = await fetch("/api/day/new", {
          method: "POST",
          body: JSON.stringify({
            userId: data2,
            employee_name: "no need",
            cost: 750,
            isCompleted: false,
            hours:0,
          }),
      ...
      <div className='container video-qr'>
        <div className='container video-qr'>
          ...
          />
          {
            scanned ?
              (
                <button type='button' onClick={clear} className='black_btn'>
                  Clear
                </button>
              )
              :
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-500">
                          Are You Sure u want to enter the attendence.
                        </p>
                      </div>
                    </div>
                  </div>
